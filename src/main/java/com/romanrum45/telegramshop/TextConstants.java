package com.romanrum45.telegramshop;

public class TextConstants {

    /**
     * Main menu
     */
    public final static String BUY_COMMAND = "Купить";
    public final static String GOODS_COMMAND = "Наличие товаров";
    public final static String ACCOUNT_COMMAND = "Личный кабинет";
    public final static String BALANCE_COMMAND = "Баланс";
    public final static String HELP_COMMAND = "Помощь";
    public final static String RULES_COMMAND = "Правила";
    public final static String MAIN_COMMAND = "Вернуться в главное меню";

    public final static String USE_KEYBOARD = "Воспользуйтесь клавиатурой";

}
